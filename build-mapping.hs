...
beeWord w =
  if length w < 4 || elem 's' w then Nothing
  else
    let letterSet = removeDupsSorted w
    in
      if length letterSet > 7 then Nothing
      else Just (w, letterSet)


beePangram :: String -> Maybe (String, String) 
beePangram w =
...
beeLines input =
  let 
    ...
    -- matches = filter beePangram allLines
